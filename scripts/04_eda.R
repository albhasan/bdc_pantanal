...
sits_samples_tb <- samples_file %>%
    ...
    dplyr::filter(id %in% c("040049", "040050", "040051", "040052", "040053",
                            "041049", "041050", "041051", "041052", "041053",
                            "042050", "042051", "042052")) %>%
    dplyr::select(-id) %>%
    (function(x){
        x %>%
...
sits_samples_tb %>%
    saveRDS(file = "./data/samples/samples_pantanal.rds")
